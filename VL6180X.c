...
uint8_t address[]={0x29,0x29,0x29,0x29,0x29,0x29}, 
    idModel[6],
    idModelRevMajor[6],
	idModelRevMinor[6],
	idModuleRevMajor[6],
	...
	idDate[6],
	idTime[6];
int distances[]={0,-1,-1,-1,-1,0};
int	diffdistance = 0;
bool ready[6];
int shdnPins[] = {2,3,6,7,8,9,12,18};//6x laser, gyro, color
...
int8_t vl6180x_get_distance_nowait(uint8_t N){
	if(ready[N]){
		// Start Single shot mode
		if(vl6180x_get_register(N, __VL6180X_RESULT_INTERRUPT_STATUS_GPIO)){
			distances[N] = vl6180x_get_register(N, __VL6180X_RESULT_RANGE_VAL);
			vl6180x_set_register(N, __VL6180X_SYSTEM_INTERRUPT_CLEAR, 0x07);
			vl6180x_set_register(N, __VL6180X_SYSRANGE_START, 0x01);
			return 1;
